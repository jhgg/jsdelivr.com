...
$directory        = 'files';
$plugin_directory = array_diff(scandir($directory), array(
    '..',
    '.',
    'info.ini'
));
foreach ($plugin_directory as &$plugin_dir) {
    ...
    $version_directory = array_diff(scandir("files/$plugin_dir"), array(
        '..',
        '.',
        'info.ini'
    ));
    ...
    foreach ($version_directory as &$ver_dir) {
        ...
        if ($ver_dir)
            $result = find_all_files("files/$plugin_dir/$ver_dir/", $plugin_dir, $ver_dir);
